        ...
        def run(self):
		        ...
		        #       from the current memory store
                	if (self._association.frequency >= self._threshold):
                                try:
                                        self._from_memory.queue.remove(self._association, True, self._wait_time)
                                except:
                                        pass
        	        ...
        	        #       back in
	                        ...
	                        self._from_memory.queue.update(i,self._association, True, self._wait_time)
        	                self._association = None
		        ...
		        #       "to" memory store           
                	if (self._association != None):
                                try:
                                        self._to_memory.queue.put(self._association, True, self._wait_time)
                                except:
                                        pass
	                        self._association = None
